...
struct ggml_object {
    size_t offs;    //保留真正数据结构的开始地址在mem_buffer中的偏移量
    size_t size;    //数据size大小

    struct ggml_object * next;

    enum ggml_object_type type;     //保存接下来的object的真实类型
...
struct ggml_context {
    ...
    struct ggml_object * objects_begin; //ggml使用 ggml_object做为元数据用来管理数据结构
...
struct ggml_context * ggml_init(struct ggml_init_params params) {
    ...
    ggml_critical_section_end(); //解锁

    struct ggml_context * ctx = GGML_MALLOC(sizeof(struct ggml_context));  //动态内存申请
...
struct ggml_tensor * ggml_get_rel_pos(
    ...
    const int64_t ne[4] = { a->ne[0], kh, qh, 1, }; //取相对tensor时维度会发生变化
...
static void ggml_compute_backward(
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_MUL: {
            if (src0_needs_grads) {
                ggml_add_or_set(ctx, cgraph, isrc0, ggml_mul(ctx, grad, src1));  //此处的ggml_mul会新建一个new tensor
            }
            if (src1_needs_grads) {
...
static void ggml_visit_parents(struct ggml_cgraph * cgraph, struct ggml_tensor * node) {
    ...
    if (node->op == GGML_OP_NONE && !(node->flags & GGML_TENSOR_FLAG_PARAM)) {
        ...
        cgraph->leafs[cgraph->n_leafs] = node;  //记录叶节点的节点位置
        cgraph->n_leafs++;
    } else {
...
static void ggml_build_forward_impl(struct ggml_cgraph * cgraph, struct ggml_tensor * tensor, bool expand) {
    ...
    ggml_visit_parents(cgraph, tensor);     //遍历所有tensor结果，从最底层到最上层依次排序，
