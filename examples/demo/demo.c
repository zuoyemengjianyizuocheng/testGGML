...
int main(void) {
    ...
    size_t ctx_size = 0;
    ctx_size += sizeof(data); // tensor a matrixsize
    ...
    struct ggml_tensor* tensor_a = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, sizeof(data)/sizeof(float));
    memcpy(tensor_a->data, data, ggml_nbytes(tensor_a));
